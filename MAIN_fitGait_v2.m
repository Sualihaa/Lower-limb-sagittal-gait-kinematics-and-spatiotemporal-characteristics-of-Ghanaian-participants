...
clc;
warning ('off','all');
cd('C:\Users\USER\Documents\fmoissenet-PatternGenerator-2315600');
addpath('C:\Users\USER\Documents\fmoissenet-PatternGenerator-2315600');
% addpath('C:\Users\USER\Documents\fmoissenet-PatternGenerator-2315600\Data');
addpath('C:\Users\USER\Documents\multipleregression_datadescriptor\Data_OURS');
...
V = 3; % number of walking speed conditions
...
save(fullfile(pwd,'Results','Vf_Settings.mat'), ...
    'minVf','maxVf','stepVf','vfPlotMax','vfStats');

VfSummary = struct2table(rmfield(vfStats,'values'));
writetable(VfSummary, fullfile(pwd,'Results','Vf_Settings.csv'));
...
%%
clearvars -except N C V T J minVf maxVf stepVf vfPlotMax vfStats pReg File Joint icycle isubject correlations
...
%     X = [Sort.walkingSpeed'];
    ...
    % =====================================================================
    clearvars -except N C V T J minVf maxVf stepVf vfPlotMax vfStats pReg File Joint icycle isubject rsubject Predictors s subject_sub Validation correlations    
...
vf_values = minVf:stepVf:0.75;

% Only keep as many velocity bins as Population.RMSE actually contains
vf_values = vf_values(1:min(numel(vf_values), nPopBins));

% for vf = minVf:stepVf:0.75%maxVf
for vf = minVf:stepVf:vfPlotMax
    temp1 = [];
    temp2 = [];
    temp3 = [];
    temp4 = [];
...
if ~isempty(population_mean)
    corridor(population_mean', population_std', minVf, stepVf, vfPlotMax, 'black');
end
% if ~isempty(population_mean)
...
saveCurrentFigure(gcf, fullfile(pwd,'Results','Figures'), ...
    sprintf('%s_validation_summary', Joint(J).name));
